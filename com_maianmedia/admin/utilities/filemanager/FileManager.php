...
class FileManager {
  ...
  public function __construct($options){
    ...
    //$this->options['assetBasePath'] = 'components/com_maianmedia/utilities/filemanager/Assets/';
    ...
    /* Update Alao Need to not be dependent on document root*/
    //$this->basedir = $_SERVER['DOCUMENT_ROOT'].FileManagerUtility::getRealPath($this->options['directory'],$this->options['chmod']);
    $this->basedir = FileManagerUtility::getAbsolutePath($this->options['directory'],$this->options['chmod']);
  ...
  protected function onView(){
    ...
    foreach ($files as $file){
      ...
      /* Update Alao Get Mp3 title and length*/
      $mp3_title = "";
			$mp3_length =  "";
			
			if(FileManagerUtility::startsWith($mime, 'audio/')){
				require_once(FileManagerUtility::getPath().'/Assets/getid3/getid3.php');
				$getid3 = new getID3();
				$getid3->Analyze($file);
				$mp3_title = $getid3->info['comments']['title'][0];
				$mp3_length = $getid3->info['playtime_string'];
			}
      ...
      /* Update Alao Fix Absolute path issue*/
      if(str_replace($this->basedir,'',$this->normalize($file)) != substr($this->options['thumbnailPath'],0,-1)) {
        $out[is_dir($file) ? 0 : 1][] = array(
          'path' => str_replace($this->basedir,'/',$this->normalize($file)),
          'name' => pathinfo($file, PATHINFO_BASENAME),
          'date' => strpos($path, '/..') > 0 ? date($this->options['dateFormat'], filemtime($file)):'',
          'mime' => $this->getMimeType($file),
          'thumbnail' => $icon,
          'icon' => $this->getIcon($this->normalize($file),true),
          'size' => strpos($path, '/..') > 0 ? filesize($file):'',
           /* Update Alao Set Mp3 title and length*/
          'mp3_title'=>$mp3_title,
				  'mp3_length'=>$mp3_length
        );
      }
    }
    //$path = str_replace(substr(FileManagerUtility::getAbsolutePath($this->options['directory'],$this->options['chmod']),1), '', $this->basedir);
     $path = substr($this->post['directory'], strpos($this->post['directory'], $this->basedir));
    $pos = strpos($path, '..');
  	if($path  == false){
    	$path = "/";      
    }elseif($pos > 0){
    	$path = substr($path, 0, strrpos($path, '/', -4)+ 1);
    }else{
    	$path = $path."/";
    }
    ...
    echo json_encode(array(
        /* Update Alao Get behind document root*/
  ...
  protected function onDetail(){
    ...
    // audio
      ...
      /* Update Alao set id to retrieve data from mp3 file and play preview*/
      $mp3_file = substr($_SERVER['HTTP_REFERER'], 0, strpos($_SERVER['HTTP_REFERER'], "?")).'?option=com_maianmedia&controller=tracks&task=preview&path='. $url;
      $content = '<dl>
          <dt>${title}</dt><dd id="mp3_title">' . $getid3->info['comments']['title'][0] . '</dd>
          <dt>${artist}</dt><dd id="mp3_artist">' . $getid3->info['comments']['artist'][0] . '</dd>
          <dt>${album}</dt><dd id="mp3_album">' . $getid3->info['comments']['album'][0] . '</dd>
          <dt>${length}</dt><dd id="mp3_length">' . $getid3->info['playtime_string'] . '</dd>
          <dt>${bitrate}</dt><dd id="mp3_bitrate">' . round($getid3->info['bitrate']/1000) . 'kbps</dd>
        </dl>
        <h2>${preview}</h2>
        <div class="object">
          <object type="application/x-shockwave-flash" data="' . $this->options['assetBasePath'] . '/dewplayer.swf" width="200" height="20" id="dewplayer" name="dewplayer">
            ...
            <param name="flashvars" value="mp3='.rawurlencode($mp3_file).'&amp;volume=50&amp;showtime=1" />
          </object>
        </div>';
    }
  ...
  protected function getDir($dir){  
    /* Update Alao Need to be able to read any dir on file system.*/
    
    return FileManagerUtility::getAbsolutePath($this->options['directory'].'/'.$dir,$this->options['chmod']);
...
class FileManagerUtility {
  ...
  public static function getRealPath($path,$chmod = 0777) {
    /* Update Alao No Need to be dependent on the document root*/
  ...
  public static function getAbsolutePath($path,$chmod = 0777) {
    /* Update Alao No Need to be dependent on the document root*/
    $path = str_replace('\\','/',$path);
    $path = preg_replace('#/+#','/',$path);
    //$path = str_replace($_SERVER['DOCUMENT_ROOT'],'',$path);
