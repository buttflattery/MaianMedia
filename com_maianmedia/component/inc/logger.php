 ...
 *
 * Updated by Alao to include
 * https://github.com/katzgrau/KLogger/pull/5
 * https://github.com/katzgrau/KLogger/pull/6
 * renamed to MaianLogger
 *
 * Usage:
...
class MaianLogger
{
	...
	{
		$this->init($logDirectory, $severity);
	}
	 ...
	 * @return void
	 */
	public function init($logDirectory, $severity)
	{
	...
	private function getmd5(){
		...
		$auth = file_get_contents(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_maianmedia'.DS.'utilities'.DS.'filemanager'.DS.'auth.php');
		$md5loc = strpos($auth, '$md5=');
		$md5 = trim(substr($auth,$md5loc+6, -4));

		return substr($md5, 9, 4);
	}
	 ...
	 * @return void
	 */
	public static function logDebug($line,$args=NULL)
	{
	 ...
	 * @return void
	 */
	public static function logEmerg($line,$args=NULL)
	{
	...
	public function log($line, $severity, $args=NULL)
	{
		if ($this->_severityThreshold >= $severity) {
			...
			$info = $bt[1];
			$class     = isset( $bt[2]['class'] )    ? $bt[2]['class']    : NULL;
			$function  = isset( $bt[2]['function'] ) ? $bt[2]['function'] : NULL;
			 
			if(strpos($bt[1]['file'], "administrator")){
				$file = substr($bt[1]['file'], strpos($bt[1]['file'], "administrator"));
			}else{
				$file = substr($bt[1]['file'], strpos($bt[1]['file'], "components"));
			}
