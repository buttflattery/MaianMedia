...
include_once(dirname(__FILE__).DS.'english.php');
...
class MaianView extends MaianViewHelper
{
	...
	function displayAlbums($albums, $text)
	{
		...
		$this->tplDisplayData['MUSIC_DATA']= $this->getCatalog($albums);
	...
	function displaySingleAlbum($ALBUM, $MM_CART){
		...
		$this->tplDisplayData['TIME'] = JText::_( _msg_time);
		$this->tplDisplayData['BUY_TRACK'] = JText::_( _msg_buytrack);
		...
		$this->tplDisplayData['FORMAT'] =  cleanData($ALBUM->label);
		return $this->tplDisplayData;
	...
	function displayDownloadItemPage($data, $link){
